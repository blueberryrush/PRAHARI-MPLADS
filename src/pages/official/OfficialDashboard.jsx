...
export default function OfficialDashboard() {
  ...
  const visible = scored.filter(p => {
    const f =
      filter === 'all' ||
      (filter === 'financial' && p.spentAmount > p.sanctionedAmount * 1.1) ||
      (filter === 'delay' && p.status === 'delayed') ||
      (filter === 'duplicate' && p.id === 'PRJ002');
  ...
  return (
    <div className="page-content command-page">
      ...
      <div className="kpi-grid">
        {[
          ['1,248', t('dash_kpi_works'), t('dash_kpi_works_sub'), 'neutral'],
          [String(priority || 87), t('dash_kpi_priority'), t('dash_kpi_priority_sub'), 'critical'],
          ['18', t('dash_kpi_verification'), t('dash_kpi_verification_sub'), 'amber'],
          ['64', t('dash_kpi_resolved'), t('dash_kpi_resolved_sub'), 'sage'],
          [`₹${(exposure / 10000000).toFixed(1)} Cr`, t('dash_kpi_exposure'), t('dash_kpi_exposure_sub'), 'teal'],
        ].map(([v, l, s, c]) => (
          <div className="kpi-panel" key={l}>
            <div className={`kpi-accent ${c}`} />
      ...
      <div className="command-grid">
        ...
        <section className="panel priority-panel">
          <div className="panel-head">
            ...
            <button className="ghost-action" onClick={() => navigate('/official/investigation')}>
              {t('btn_open_investigation')} <ArrowRight size={15} />
            </button>
          </div>
        ...
        <aside className="side-stack">
          <section className="panel insight-panel">
            ...
            <div className="insight-item">
              ...
              <div>
                ...
                <button onClick={() => navigate('/official/investigation')}>
          ...
          <section className="panel action-panel">
            ...
            <div className="action-line">
              ...
              <div><b>7</b><span>{t('dash_action_high')}</span></div>
              <ArrowRight size={14} />
            </div>
            <div className="action-line">
              <span className="queue-dot amber" />
              <div><b>6</b><span>{t('dash_action_field')}</span></div>
              <ArrowRight size={14} />
            </div>
            <div className="action-line">
              <span className="queue-dot neutral" />
              <div><b>5</b><span>{t('dash_action_evidence')}</span></div>
              <ArrowRight size={14} />
            </div>
          </section>
        </aside>
        ...
        <section className="panel map-panel">
          <div className="panel-head">
            ...
            <span className="map-caption">
              <span className="legend-dot high" /> {t('dash_legend_priority')}
              <span className="legend-dot med" /> {t('dash_legend_review')}
              <span className="legend-dot safe" /> {t('dash_legend_stable')}
            </span>
          </div>
          <SpatialCommandView
            projects={scored}
